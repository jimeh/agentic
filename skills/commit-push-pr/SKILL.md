...
Template detection is internal workflow state. Do not include template status
metadata in the PR title or body, including lines like `No PR template found`,
`PR template: No PR template found`, or the path of the selected template unless
the repository's template explicitly asks for it.
...
- **Title**: use conventional commits format when the repo follows that
  convention. Keep the title to one clear line that describes the branch's
  purpose, not the mechanics of creating the PR. Prefer wording that would read
  well as a bullet in a user-facing changelog when the change has user-visible
  impact. For internal-only changes, frame the title around the durable project
  outcome rather than the implementation task.
...
- **Manual QA**: include manual QA steps only when they are useful, concrete,
  and tied to the behavior changed by the branch. Prefer reviewer workflows or
  user-visible scenarios over commands. Do not fill this section with generic
