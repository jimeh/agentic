...
retain its `default_branch` and `base` bindings before changing repository or
remote state. Stop if the base remains ambiguous.
...
Refresh the resolved remote-tracking ref, then read every change the branch
carries using the bound `base`, since the PR describes the branch and not the
latest commit:
